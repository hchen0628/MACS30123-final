...
- This table displays the Wasserstein distances between the distributions of real and simulated data across various features. The Wasserstein distance quantifies the differences between the distributions of generated and actual data by measuring the minimum cost flow between them. A smaller value indicates a higher approximation to the real dataset, thus enabling the estimation accuracy of the simulated dataset to be inferred. The comparison shows that features such as "sex," "race," and "citizen" have very small Wasserstein distances, indicating that the WGAN model excels in capturing the distribution of these categorical variables and is very close to the actual data. Additionally, although "famsize" and "sect" exhibit higher Wasserstein distances, indicating some deviation from the real data, they are still within an acceptable range. With further tuning, the model still has the potential to support the construction of counterfactual simulation data for complex attributes such as health status and industry sectors.

## Jensen-Shannon Divergence
![](https://github.com/hchen0628/MACS30123-final/blob/main/Data%20visualization/js_divergence_table.png)
- This table displays the Jensen-Shannon divergence values between the real and simulated datasets across various features. Jensen-Shannon divergence is a method for measuring the similarity between two probability distributions. It is symmetric and has finite values, making it a reliable metric for comparing dataset similarities. Smaller values indicate higher similarity between distributions. Features like "sex" and "citizen" have very low divergence, suggesting that the WGAN model effectively captures these distributions with high fidelity. However, "famsize" and "health" exhibit higher divergence. While these differences are not severe, the model's performance estimating these variables, particularly "famsize," as indicated by Wasserstein distance and Jensen-Shannon divergence, may require further refinement to more closely approximate the real data distributions.
